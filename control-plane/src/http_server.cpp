...
void HttpSession::handle_request()
{
    ...
    if (method == http::verb::post && target == "/nodes/register")
    {
        ...
        try
        {
            ...
            {
                make_error(http::status::bad_request, "node_id out of range");
                return;
            }

            Node node;
            node.node_id = static_cast<NodeId>(raw_node_id);
    ...
    if (method == http::verb::post && target == "/nodes/heartbeat")
    {
        ...
        try
        {
            const auto raw_node_id = obj.at("node_id").as_int64();
            ...
            const auto node_id = static_cast<NodeId>(raw_node_id);
    ...
    if (method == http::verb::post && target == "/nodes/drain")
    {
        ...
        try
        {
            ...
            const auto node_id = static_cast<NodeId>(raw_node_id);
            registry_.set_node_draining(node_id);

            response.result(http::status::ok);

            json::object body;
            body["status"] = "node_draining";
            body["node_id"] = raw_node_id;
    ...
    if (method == http::verb::delete_ && target == "/nodes/remove")
    {
        ...
        registry_.force_remove_node(node_id);

        response.result(http::status::accepted);

        json::object body;
        body["status"] = "node_removed";
        body["node_id"] = raw_node_id;
